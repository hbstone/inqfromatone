...
function handleLogin(socket, input) {
  ...
  if (!input) {
    writeToSocket(
      socket,
      "Name cannot be blank. Please enter your character's name:",
    );
    return;
  }
...
function handlePassword(socket, input) {
  ...
  if (socket.character.new) {
    ...
    if (characters[socket.character.name].password === hashedPassword) {
      ...
      writeToSocket(
        socket,
        `You are now in ${world.rooms[1].name}.\n${world.rooms[1].description}`,
      );
    } else {
...
function handleCommand(socket, input) {
  ...
  if (input === "look") {
    ...
    const target = currentRoom.occupants.find(
      (client) => client.character.name === targetName,
    );
    if (target) {
      writeToSocket(
        socket,
        `You look at ${targetName}: ${target.character.description}`,
      );
    ...
    broadcast(
      currentRoom,
      `${socket.character.name} says, "${message}"`,
      socket,
    );
  } else if (input.startsWith("tell ")) {
    const [recipientName, ...messageParts] = input.slice(5).split(" ");
    const message = messageParts.join(" ");
    const recipientSocket = Array.from(world.getAllSockets()).find(
      (client) => client.character.name === recipientName,
    );
    if (recipientSocket) {
      writeToSocket(
        recipientSocket,
        `${socket.character.name} tells you, "${message}"`,
      );
      writeToSocket(socket, `You tell ${recipientName}, "${message}"`);
    } else {
      writeToSocket(
        socket,
        `No one named ${recipientName} is currently connected.`,
      );
    }
  } else if (input.startsWith("whisper ")) {
    const [recipientName, ...messageParts] = input.slice(8).split(" ");
    const message = messageParts.join(" ");
    const recipientSocket = currentRoom.occupants.find(
      (client) => client.character.name === recipientName,
    );
    if (recipientSocket) {
      writeToSocket(
        recipientSocket,
        `${socket.character.description} whispers to you, "${message}"`,
      );
      writeToSocket(socket, `You whisper to ${recipientName}, "${message}"`);
    } else {
      writeToSocket(
        socket,
        `You don't see anyone named ${recipientName} here.`,
      );
    }
