...
const world = {
  ...
  getAllSockets() {
    return Object.values(this.rooms).reduce(
      (all, room) => all.concat(room.occupants),
      [],
    );
  },
  moveToRoom(socket, roomId, direction) {
    ...
    broadcast(
      currentRoom,
      `${socket.character.name} leaves ${direction}.`,
      socket,
    );
    broadcast(
      nextRoom,
      `${socket.character.name} enters from the ${direction}.`,
      socket,
    );
    writeToSocket(
      socket,
      `You are now in ${nextRoom.name}.
